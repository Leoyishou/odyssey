...
一旦你这么做,你仍然可以输入双括号(如果你想要自动完成建议,你必须这么做),它们会在完成时转换为 Markdown。
## publish

1. 官方publish 服务
	- 优点：付费使用，软件内插件，点击即可发布，非常方便
	- 缺点：无原生评论系统

2. 自己用[vuepress](vuepress.md)这个开源项目
	- 缺点：稍微繁琐些，需要一定的计算机基础
	- 优点：更加 DIY 一些，免费，搭建好[CICD](CICD.md)之后再做一个本地 Obsidian 插件，可以获得和官方服务类似的丝滑体验。
		- 细节一：用 Linter 插件，为所有md 文件生成 frontmatter，把文件名作为 title，避免 vuepress 把 md 转 html 时候出现的标题乱码
		- 细节二：用 link-converter 插件把过去的 wikilink 式的双链格式转为 markdown 格式，实现在线发布后能获得本地一样的双链跳转体验，同时在设置中disable Settings > Files & Links > Use Wikilinks，这样以后本地的`[[`语法生成的也是 markdown 格式的link
...
按照 [Corrective feedback Loop](Corrective%20feedback%20Loop.md)，需要找一些 role model 来给自己反馈
...
- [2023-03-03](2023-03-03) 更新：
	- 一个英语单词分类整理库，包含几千条笔记。作者说自己花了 7 年时间整理的，后挪到 obsidian 中。我看了一下分类整理很全面，有个缺憾是里面的单词没有进行双链，只使用到了 obsidian 做为 markdown 编辑器以及文件管理器的功能。
...
- [2023-03-06](2023-03-06) 更新:
	- 作者是：及时春雨，logseq 和 obsidian 联用示例库。
		- 关注其公众号获取，有很多不曾见过的奇技淫巧，已经持续更新 1 年至 v3。
	- obsidian 工具箱，作者是 [「回归 Obsidian 的纯与真，写给普通人的入门指南」](https://sspai.com/post/72697) 文章的作者
		- 下载地址：[OB 工具箱.zip](https://www.jianguoyun.com/p/DejxOSUQkdv8Bhjrja8E)

## 批量移动文件

用 python 脚本完成 `/Users/liuyishou/usr/projects/viva-backend/util/file_move.py`
...
![]([HTTPS](HTTPS.md)://cubox.pro/c/filters:no_upscale()?imageUrl=https%3A%2F%2Fmiro.medium.com%2Fmax%2F1400%2F1*zrsbKmvBdHsY9K8UF44sxw.png)
