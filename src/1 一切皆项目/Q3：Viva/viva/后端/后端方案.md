...
1. HTTP 请求到达 `essay_controller.py` 中的 `process_essay` 函数。
2. `process_essay` 函数调用 `ProcessEssayUseCase.execute` 方法。
3. `ProcessEssayUseCase.execute` 方法执行以下步骤：
   a. 创建 `Essay` 实体  
   b. 调用 `EssayRepository.save` 保存 Essay  
   c. 调用 `EssayProcessingService.process_essay` 处理 Essay  
   d. 调用 `MappingCardRepository.save_all` 保存生成的 MappingCards  
   e. 返回处理结果

4. `EssayProcessingService.process_essay` 方法执行以下步骤：
   a. 调用 `SegmentationService.validate_essay` 验证 Essay 内容  
   b. 调用 `SegmentationService.process_essay` 处理 Essay 内容  
   c. 根据处理结果创建 MappingCard 实体

5. `SegmentationService.process_essay` 方法执行以下步骤：
   a. 调用 `split_into_sentences` 将 Essay 分割成句子  
   b. 对每个句子调用 `segment_sentence` 进行分词

6. 处理结果通过各层逐级返回，最终由 `essay_controller.py` 返回 HTTP 响应。
...
1. 依赖注入设置：
   ...
   这部分代码创建了应用所需的各种服务和仓储的实例。这相当于在 Spring Boot 中使用 `@Bean` 注解定义 beans。
...
2. 依赖注入函数：
   ...
   这个函数定义了如何创建 `ProcessEssayUseCase` 实例。它类似于 Spring Boot 中的一个工厂方法。
...
3. 路由包含：
   ...
   这行代码将 essay 相关的路由包含到主应用中，类似于 Spring Boot 中的 `@ComponentScan` 或明确导入某个控制器。
...
4. 依赖覆盖：
   ...
   这行代码告诉 FastAPI 如何解析 `ProcessEssayUseCase` 的依赖。当某个路由需要 `ProcessEssayUseCase` 时，FastAPI 会调用 `get_process_essay_use_case` 函数来获取实例。这在 Spring Boot 中类似于使用 `@Autowired` 注解。
...
- CI
	- SCP 协议传文件到服务器
	- `scp -i ~/keys/my-aws-key.pem ~/projects/myapp/target/myapp.jar ec2-user@203.0.113.0:/home/ec2-user  
`
	- 最简单的：写个脚本  
		次之：gitlab cicd 集成  
		再次：Jenkins
...
要使用 Git 钩子（hook）触发 `git pull` 操作，您通常需要在服务器端设置一个 `post-receive` 钩子。这个钩子在服务器上的 Git 仓库接收到新的提交后触发。下面是设置过程的简要说明：

#### 在服务器端仓库设置钩子

1. **定位 Git 仓库**：导航到您的 Git 仓库的 `hooks` 目录。这通常在仓库的 `.git/hooks` 目录下。
...
2. **创建或编辑钩子**：在 `hooks` 目录中创建一个名为 `post-receive` 的文件（如果不存在的话）。
...
3. **编写钩子脚本**：使用您喜欢的文本编辑器编辑 `post-receive` 文件，添加必要的脚本来执行 `git pull` 操作。示例如下：
    ...
    `#!/bin/sh GIT_WORK_TREE=/path/to/working/directory git pull origin master`

    

    确保将 `/path/to/working/directory` 替换为您希望执行 `git pull` 的目录的路径，并将 `origin master` 替换为相应的远程名称和分支（如果有必要）。

    
4. **使钩子可执行**：更改 `post-receive` 文件的权限，使其成为可执行文件。
...
使用 Git 钩子自动化 `git pull` 操作可以是一个强大的工具，但请谨慎使用，并确保您完全理解其工作原理和潜在的影响。
